...
| [[AITER_FUSED_KERNELS]] | User-facing guide for the fused All-Gather+MatMul and MatMul+Reduce-Scatter kernels in `benchmarks/aiter_kernels/`, including the experimental HipKittens/Iris CDNA4 backend. API, dispatcher, tuning, troubleshooting, roofline analysis. | You need to use, tune, or debug the fused TP-linear kernels. |
