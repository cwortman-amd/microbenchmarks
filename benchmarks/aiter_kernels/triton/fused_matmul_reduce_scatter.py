...
def fused_matmul_reduce_scatter(
    ...
    *,
    ...
    if IRIS_AVAILABLE and isinstance(getattr(A, "_iris_ctx", None), object):
        ...
        partial = ctx.iris_ctx.zeros((M_shard, N), dtype=A.dtype)
        grid = (cfg["NUM_SMS"],)
        ...
        if reduce_op == "avg":
            avg_grid = (
                triton.cdiv(M_shard, cfg["BLOCK_M"]),
        ...
        if rest_shape:
            partial = partial.reshape(M_shard, *rest_shape).movedim(0, scatter_dim).contiguous()
        return partial
